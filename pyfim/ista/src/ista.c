...
#define VERSION     "version 4.16 (2016.04.09)        " \
                    "(c) 2009-2016   Christian Borgelt"
...
{                               /* --- prepare reporter for Carpenter */
  ...
  mrep = ((target & ISR_MAXIMAL) && !(mode & ISTA_FILTER))
       ? ISR_MAXIMAL : 0;       /* build reporting mode */
...
{                               /* --- intersecting transactions */
  ...
  #endif
  #ifdef ISTA_ABORT             /* if to check for interrupt */
  if (sig_aborted()) { cleanup(&a); return -1; }
...
{                               /* --- main function */
  ...
  #endif  /* #ifndef QUIET */
  /* free option characters: acjlouxyz [A-Z]\[CFNPRZ] */
