...
{                               /* --- get surrogate function code */
  ...
  if (s[0] && !s[1]) {          /* check for avalid string */
    switch (s[0]) {             /* evaluate the surrogate method code */
      case 'i': return SUR_IDENTITY;
      case 'r': return SUR_RANDOM;
      case 'p': return SUR_SWAP;
      case 'w': return SUR_SWAP;
      case 's': return SUR_SHUFFLE;
    }
  }
  ...
  while ((trans = PyIter_Next(ti)) != NULL) {
    ...
    if (dict) {                 /* if trans. multiplicities given */
      ...
      if      (PyInt_Check (mul)) w = (SUPP)PyInt_AsLong (mul);
        ...
        return ERR_TYPE("transaction multiplicities must be integer"); }
...
{                               /* --- frequent item set mining */
  ...
  thresh /= 100.0;              /* scale evaluation threshold */
  ...
  if (!isrep) { tbg_delete(tabag, 1); return ERR_MEM(); }
  ...
  if (isr_pyborder(isrep, border) == NULL) {
    isr_delete(isrep, 0); tbg_delete(tabag, 1); return ERR_RTN(); }
  ...
  ||  (isr_setup(isrep) < 0)) { /* set up the item set reporter */
    isr_delete(isrep, 0); tbg_delete(tabag, 1); return ERR_MEM(); }

  /* --- frequent item set mining --- */
  r = fpgrowth(tabag, target, smin, smin, 1, eval, agg, thresh,
               ITEM_MIN, algo, mode, 0, isrep);
...
{                               /* --- association rule mining */
  ...
  if (!tabag) return ERR_RTN(); /* get the given transactions */
  conf /= 100.0;                /* scale the minimum confidence and */
  wgt  = tbg_wgt(tabag);        /* get the total transaction weight */
  supp = (supp >= 0) ? supp/100.0 *(double)wgt *(1-DBL_EPSILON) : -supp;
...
{                               /* --- Apriori algorithm */
  ...
  if (algo < 0) return ERR_VALUE("invalid Apriori algorithm");
  ...
  /* --- create transaction bag --- */
  sig_install();                /* install the signal handler */
  tabag = tbg_fromPyObj(tracts, (target == ISR_RULES) ? appear : NULL);
  if (!tabag) return ERR_RTN(); /* get the given transactions */
  conf /= 100.0;                /* scale the minimum confidence and */
  wgt  = tbg_wgt(tabag);        /* get the total transaction weight */
  supp = (supp >= 0) ? supp/100.0 *(double)wgt *(1-DBL_EPSILON) : -supp;
...
{                               /* --- Eclat algorithm */
  ...
  thresh /= 100.0;              /* add eval. flag, scale threshold */
  if (salgo[0] && salgo[1]) {   /* if textual identifier */
    if      (strcmp(salgo, "auto")   == 0) salgo = "a";
  ...
  if (algo == ECL_AUTO) {       /* if automatic variant choice */
    ...
    algo = ((target & (ISR_CLOSED|ISR_MAXIMAL))
        && ((double)tbg_extent(tabag) /((double)m*(double)wgt) > 0.02))
...
{                               /* --- Apriori algorithm */
  ...
  siglvl /= 100.0;              /* scale significance level */
...
{                               /* --- Accretion algorithm */
  ...
  if (siglvl <= 0) return ERR_VALUE("siglvl must be positive");
  siglvl /= 100.0;              /* scale the significance level */
...
{                               /* --- generate a pattern spectrum */
  ...
  tabag = tbg_fromPyObj(tracts, NULL);
  if (!tabag) return ERR_RTN(); /* get the given transactions */
  if ((surr == SUR_SHUFFLE) && !tbg_istab(tabag)) {
  ...
  supp = (supp >= 0) ? supp/100.0 *(double)wgt *(1-DBL_EPSILON) : -supp;
  smin = (SUPP)ceilsupp(supp);  /* compute absolute support value */
  psp  = fpg_genpsp(tabag, target, smin, (ITEM)zmin, (ITEM)zmax,
...
{                               /* --- estimate a pattern spectrum */
  ...
  psp = fpg_estpsp(tabag, target, smin, (ITEM)zmin, (ITEM)zmax,
...
#define FIM_DESC \
  ...
  "version 6.22 (2016.05.02)     (c) 2011-2016   Christian Borgelt"
